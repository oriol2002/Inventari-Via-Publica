...
const App: React.FC = () => {
  ...
  const handleSaveCrossing = async (crossing: PedestrianCrossing) => {
    ...
    try {
      ...
      alert("No s'ha pogut sincronitzar amb el servidor. El canvi es mantindrà localment fins que recarreguis.");
    }
  };

  // Funció per a Punts de Control Ràpids (Checkpoint)
  const handleQuickUpdate = async (id: string, updates: Partial<PedestrianCrossing>) => {
    ...
    try {
      ...
      console.error("Error en punt de control:", error);
    }
  };
  ...
  const handleCreateReport = async (selectedCrossings: PedestrianCrossing[], type: 'maintenance' | 'technical' | 'statistical') => {
    ...
    try {
      ...
      alert("No s'ha pogut guardar l'informe al servidor.");
    } finally {
  ...
  const alerts = filteredCrossings.filter(c => {
    // Check de data efectiva (revisió o pintat)
  ...
  return (
    <div className="h-full flex flex-col bg-slate-100 overflow-hidden font-sans relative">
      {isGeneratingReport && (
