      ...
      try {
        ...
        const serverData: PedestrianCrossing[] = snapshot.docs.map(docSnap => {
          ...
          return {
            ...
            signDetail: row.signDetail || row.sign_detail || undefined,
            retentionLineLength: row.retentionLineLength || row.retention_line_length || undefined,
            crossingWidth: row.crossingWidth || row.crossing_width || undefined,
            image: row.image || '',
    ...
    try {
      ...
      const serverData = (data || []).map(row => {
        ...
        return {
          ...
          signDetail: row.sign_detail || row.signDetail || undefined,
          retentionLineLength: row.retention_line_length || row.retentionLineLength || undefined,
          crossingWidth: row.crossing_width || row.crossingWidth || undefined,
            image: row.image || '',
  ...
  async save(crossing: PedestrianCrossing): Promise<void> {
    ...
    if (BACKEND === 'firebase') {
      const payload = {
        ...
        signDetail: crossing.signDetail || null,
        retentionLineLength: crossing.retentionLineLength || null,
        crossingWidth: crossing.crossingWidth || null,
    ...
    try {
      const payload = {
        ...
        asset_subtype: crossing.assetSubType || null,
        sign_detail: crossing.signDetail || null,
        retention_line_length: crossing.retentionLineLength || null,
        crossing_width: crossing.crossingWidth || null,
        image: crossing.image,
