...
const CrossingForm: React.FC<Props> = ({ initialData, onClose, onSubmit, city, onImageCapture }) => {
      ...
      if ('geolocation' in navigator) {
        navigator.geolocation.getCurrentPosition(
          ...
          (err) => {
            ...
            // Fallback a Tortosa centre si falla el GPS
        ...
        // Fallback si no hi ha suport de geolocalització
        const defaultLoc = { lat: 40.8125, lng: 0.5216 };
        setLocation(defaultLoc);
        shouldRecenter.current = true;
      }
  ...
  useEffect(() => {
    ...
    if (showImageEditor || !image || !location || !mapContainerRef.current || mapRef.current) return;
    ...
    const resizeInterval = setInterval(() => map.invalidateSize(), 500);
    setTimeout(() => clearInterval(resizeInterval), 2000);

  }, [image, showImageEditor, location]); // Afegit 'location' a dependencies
  ...
  const handleStreetChange = (val: string) => {
    setLocation(prev => prev ? ({ ...prev, street: val }) : { lat: 0, lng: 0, street: val });
  ...
  const selectStreet = (suggestion: any) => {
    ...
    const newLoc = { 
      ...
      neighborhood: addr.suburb || addr.neighbourhood || location?.neighborhood || '',
  ...
  return (
    <div className={!image ? "p-8" : "p-6 h-full overflow-y-auto"}>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 pb-4">
          ...
          <div className="bg-slate-50 p-5 rounded-[2.5rem] border border-slate-300 space-y-5">
             ...
             <div className="space-y-0.5">
                ...
                <select value={location?.neighborhood || ''} onChange={(e) => setLocation(prev => prev ? ({ ...prev, neighborhood: e.target.value }) : { lat: 0, lng: 0, neighborhood: e.target.value })} className="w-full bg-white border border-slate-300 rounded-xl p-3.5 text-[12px] font-black uppercase outline-none focus:ring-2 focus:ring-blue-500/20 appearance-none text-slate-700">
             ...
             <div className="relative group -mx-5 overflow-hidden border-y border-slate-300 shadow-xl bg-slate-200">
               <div ref={mapContainerRef} className="h-80 w-full z-0"></div>
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest pl-1">Data darrera Actuació</label>
          ...
          <button 
            ...
            className="w-full bg-blue-600 text-white py-5 rounded-[2rem] font-black text-[12px] uppercase tracking-[0.2em] shadow-2xl shadow-blue-500/20 disabled:opacity-30 hover:bg-blue-700 transition-all active:scale-[0.98]"
          >
